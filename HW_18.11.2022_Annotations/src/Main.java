...
//        First level: Создать класс, который может выполнять «тесты», в качестве тестов выступают классы
...
//        с которыми будет выбираться порядок их выполнения, если приоритет одинаковый то порядок не имеет значения.
...
//        Примечание: это домашнее задание никак не связано с темой тестирования через JUnit и использованием
//        этой библиотеки, то есть проект пишется с нуля.
        Class c = Tests.class;
        RunTests.start(c);
        System.out.println();
        RunTests.start(c.getSimpleName());
